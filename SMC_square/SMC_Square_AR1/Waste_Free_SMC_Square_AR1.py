...
from initialization_AR1 import initialize_from_posterior
...
class WF_SMC_Square:
    ...
    """
    def __init__(self, p, logfile, rho =1, N_theta=50, N_x=300, N_rejuvenate=5, threshold=0.4):
        """
        Initializes the WF_SMC_Square sampler with model parameters and SMC settings.
        
        Args:
            p (int): The number of lags for the VAR model[cite: 8].
    ...
    def _rejuvenate_one_theta(self, i, rejuvenate_A, tracker):
        ...
        for j in range(self.N_rejuvenate):
            ...
            for k_unobs in range(self.current_k + 1, self.K):
                # Simulate future log-lambda at (current_t, k_unobs) 
                # based on AR1 model
                fix_log_lambda_i[k_unobs, self.current_t] = self.mu[k_unobs] + self.rho * (fix_log_lambda_i[k_unobs, self.current_t - 1]-self.mu[k_unobs]) + np.sqrt(Phi_i[k_unobs]) * np.random.normal(0,1)
    ...
    def run(self, y, t_init=0, init_result=None, if_pred=False):
        ...
        if t_init == 0:
            ...
            if init_result is None:
                ...
                init_result = initialize_from_posterior(y, self.p, t_init, self.N_rejuvenate, self.N_x, self.M_theta, self.rho)
    ...
    def _smc_inner_loop(self, i, t, k, Phi_k, A_k, Api_k, y_t, z_t):
        ...
        log_lambda_t = self.mu[k] + self.rho * (log_lambda_prev - self.mu[k]) + np.sqrt(Phi_k[i]) * np.random.normal(size=self.N_x)
