...
class WF_SMC_Square:
    ...
    def init_prior(self, y):
        ...
        self.trajectories = [
            [lil_matrix((self.N_x, self.T + 1), dtype=np.float64) for _ in range(self.K)]
            for _ in range(self.N_theta)
        ]
        self.ancestors = [
            [lil_matrix((self.N_x, self.T), dtype=np.int32) for _ in range(self.K)]
            for _ in range(self.N_theta)
        ]
        ...
        for i in range(self.N_theta):
            for k in range(self.K):
                # Assignment to a column requires a 2D shape (N_x, 1)
                self.ancestors[i][k][:, 0] = np.arange(self.N_x).reshape(-1, 1)
...
############ ignore this for now
    ...
    def intialize_from_posterior_result(self, results, init_t):
        ...
        for i in range(self.M_theta):
            ...
            for j in range(self.N_rejuvenate + 1):
                ...
                for k in range(self.K):
                    self.trajectories[target_index][k][:, 0:init_t + 1] = sample_result['trajectories'][j,:,k,:]
                    self.ancestors[target_index][k][:, 0:init_t] = sample_result['ancestors'][j,:,k,:]
    ...
    def waste_free_resampling(self):
        ...
        self.trajectories[:self.M_theta] = [self.trajectories[i] for i in indices]
        self.ancestors[:self.M_theta] = [self.ancestors[i] for i in indices]
...
########### ignore this for now #############
    ...
    def clean_trajectory_single(self,t_k, traj, anc):
        """
        clean the trajacgtory of the ith particle
        both traj and anc are (N_x, t_k) matrices
        """
        offspring_counts = np.zeros((self.N_x, t_k), dtype=bool)
        for j in range(self.N_x):
            idx = j
            for t in reversed(range(t_k)):
                # not refered to if the offspring_counts is 0
                offspring_counts[idx, t] = True
                idx = anc[j, t]
        traj[~offspring_counts] = 0.0
        anc[~offspring_counts] = 0
        
        return lil_matrix(traj), lil_matrix(anc)
    ...
    def _rejuvenate_one_theta(self, i, rejuvenate_A, tracker):
        ...
        for k in range(self.K):
            # convert to dense matrix so that accessing columns are faster
            traj = self.trajectories[i][k].toarray()
            anc = self.ancestors[i][k].toarray().astype(int)
            idx = np.random.choice(self.N_x, p=weights[:, k])

            if k <= self.current_k:
                df_phi.append(self.current_t + 1)
                for t in reversed(range(self.current_t + 1)):
                    fix_log_lambda_i[k, t] = traj[idx, t + 1]
                    idx = anc[idx, t]
        ...
        for j in range(self.N_rejuvenate):
            ...
            for k in range(self.K):
                ...
                traj,anc, sampled_traj, _, _ = csmc.run(if_reconstruct=True)
                fix_log_lambda_phi_i.append(sampled_traj)
                fix_log_lambda_i[k, :t_k] = sampled_traj

                ##### add in here
                ##### cleaning trajactory 
                self.trajectories[target_idx][k][:, 1:t_k + 1], self.ancestors[target_idx][k][:, :t_k] = self.clean_trajectory_single(self,t_k, traj, anc)
...
############# ignore this for now ##########
    ...
    def _smc_inner_loop(self, i, t, k, Phi_k, A_k, Api_k, y_t, z_t):
        ...
        if t == 0:
            # CORRECTED: Read from sparse matrix and convert to dense array for calculations.
            log_lambda_prev = self.trajectories[i][k][:, t].toarray().flatten()
        else:
            # CORRECTED: Read ancestor indices and convert to dense array.
            idx = self.ancestors[i][k][:, t-1].toarray().flatten().astype(int)
            # Use the dense indices to do advanced indexing, then convert the result to dense.
            log_lambda_prev = self.trajectories[i][k][idx, t].toarray().flatten()
        ...
        # CORRECTED: Must reshape to a column vector (N_x, 1) for assignment.
        self.trajectories[i][k][:, t + 1] = log_lambda_t.reshape(-1, 1)
        ...
        if ess < self.threshold * self.N_x:
            ...
            # CORRECTED: Must reshape to a column vector for assignment.
            self.ancestors[i][k][:, t] = new_idx.reshape(-1, 1)
            self.inner_weights[i, :, k] = np.zeros(self.N_x)
        else:
            # CORRECTED: Must reshape to a column vector for assignment.
            self.ancestors[i][k][:, t] = np.arange(self.N_x).reshape(-1, 1)
