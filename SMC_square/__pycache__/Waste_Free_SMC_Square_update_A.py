...
sys.path.append('/home/onyxia/work/SMC_square')
...
class WF_SMC_Square:
    ...
    def _rejuvenate_one_theta(self, i, rejuvenate_A, tracker):
        ...
        for j in range(self.N_rejuvenate):
            ...
            for k in range(self.current_k +1, self.K):
                ...
                log_lambda = fix_log_lambda_i[k, self.current_t -1] + np.random.normal(0, Phi_i[k])
    ...
    def smc_step_single(self, t, k):
        ...
        # Clip residuals to avoid huge likelihoods (e.g., when variance is tiny)
        residuals = np.clip(residuals, a_min=-1e3, a_max=1e3)
        ...
        # use unnormalized log weight
        self.inner_weights[:,:,k] += safe_log_weights

        ll_k = scipy.special.logsumexp(self.inner_weights[:,:,k], axis=1, keepdims=True) - np.log(self.N_x)

        # Normalized log weights for ESS
        logsumexp_per_theta = scipy.special.logsumexp(self.inner_weights[:,:,k], axis=1, keepdims=True)
        logsumexp_per_theta = np.where(np.isfinite(logsumexp_per_theta), logsumexp_per_theta, 0.0)
        log_w = self.inner_weights[:,:,k] - logsumexp_per_theta
