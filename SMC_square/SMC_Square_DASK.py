...
class WF_SMC_Square:
    ...
    def intialize_from_posterior_result(self, results, init_t):
        ...
        for i in range(self.M_theta):
            ...
            for j in range(self.N_rejuvenate + 1):
                ...
                trj = np.zeros((1, self.N_x, self.K, self.T + 1))
                ans = np.zeros((1, self.N_x, self.K, self.T))
                trj[0,:,:,0:init_t + 1] = sample_result['trajectories'][j]
                ans[0,:,:,0:init_t] = sample_result['ancestors'][j]
                all_trj.append(da.from_array(trj, chunks= (1, -1, -1, -1)))#(1, N_x, K, T+1)
                all_ans.append(da.from_array(ans, chunks= (1, -1, -1, -1)))#(1, N_x, K, T)
        
        # use concatenate instead of inplace modification for da
        self.trajectories = da.concatenate(all_trj, axis = 0) #(N_theta , N_x, K, T+1)
        self.ancestors = da.concatenate(all_ans, axis = 0) #(N_theta , N_x, K, T)
        print('after initialization',self.trajectories.shape,self.ancestors.shape)
    ...
    def waste_free_resampling(self):
        ...
        # Select the M_theta chunks using da.take
        # This creates new Dask arrays with only the selected particles.
        # and then we can concate the remaining particles during rejuvenation
        self.trajectories = da.take(self.trajectories, indices, axis=0)
        self.ancestors = da.take(self.ancestors, indices, axis=0)
    ...
    def _rejuvenate_one_theta(self, i, rejuvenate_A, tracker):
        ...
        # Retrieve the parent particle's current trajectories and ancestors as NumPy arrays
        # This will trigger Dask to compute this specific chunk.
        current_theta_trajectories_np = self.trajectories[i, :, :, :].compute() # (N_x, K, T+1)
        current_theta_ancestors_np = self.ancestors[i, :, :, :].compute()     # (N_x, K, T)
        ...
        for j in range(self.N_rejuvenate):
            ...
            for k in range(self.K):
                ...
                trj, ans, sampled_traj, _, _ = csmc.run(if_reconstruct=True)
                
                # add padding so that concate work without shape mismatch
                # Pad trj_np to (N_x, max_trj_len)
                padded_trj_np = np.zeros((self.N_x, self.T + 1))
                padded_trj_np[:, 1:t_k + 1] = trj # Copy data up to t_k

                # Pad ans_np to (N_x, max_ans_len)
                padded_ans_np = np.zeros((self.N_x, self.T), dtype=int)
                padded_ans_np[:, 0:t_k] = ans # Copy data up to t_k

                all_trj_k.append(da.from_array(padded_trj_np[np.newaxis,:,np.newaxis,:], chunks = (1,-1, -1, -1))) # (1,N_x,1, self.T)
                all_ans_k.append(da.from_array(padded_ans_np[np.newaxis,:,np.newaxis,:], chunks = (1,-1, -1, -1)))
                
                fix_log_lambda_phi_i.append(sampled_traj)
                fix_log_lambda_i[k, :t_k] = sampled_traj

            ## add all_try and all_ans by concatenating them to the first axis ?
            all_trj_da.append(da.concatenate(all_trj_k, axis = 2))# (1,N_x, K, current_t+1)
            all_ans_da.append( da.concatenate(all_ans_k, axis = 2))
            #self.trajectories = da.concatenate([self.trajectories,all_trj_da], axis = 0)
            #self.ancestors = da.concatenate([self.ancestors, all_ans_da], axis = 0)
        ...
        #print('after rejuvenation',self.trajectories.shape,self.ancestors.shape)
            all_A.append(A_i)
            all_Phi.append(Phi_i)
            all_Pi.append(Pi_i)
            all_Api.append(Api_i)
        return all_trj_da, all_ans_da, all_A, all_Phi, all_Pi, all_Api
    ...
    def rejuvenate_parallelized(self, rejuvenate_A=False):
        ...
        # Unzip all returned lists of Dask arrays
        all_trj_da_list, all_ans_da_list, all_A, all_Phi, all_Pi, all_Api = zip(*results)

        # Flatten and concatenate along the first axis
        all_trj_da_concat = da.concatenate([item for sublist in all_trj_da_list for item in sublist], axis=0)
        all_ans_da_concat = da.concatenate([item for sublist in all_ans_da_list for item in sublist], axis=0)

        # Update state once
        self.trajectories = da.concatenate([self.trajectories, all_trj_da_concat], axis=0)
        self.ancestors = da.concatenate([self.ancestors, all_ans_da_concat], axis=0)

        self.A[self.M_theta:, :,:] = np.array(all_A).reshape((-1, self.K, self.K))
        self.Phi[self.M_theta:, :] = np.array(all_Phi).reshape((-1, self.K))
        self.Pi[self.M_theta:, :,:] = np.array(all_Pi).reshape((-1, self.K, self.K * self.p + 1))
        self.Api[self.M_theta:, :,:] = np.array(all_Api).reshape((-1, self.K, self.K * self.p + 1))
        self.inner_weights[self.M_theta:,:,:] = np.zeros_like(self.inner_weights[self.M_theta:,:,:])
    ...
    ######### np operation within a chunk should be fine
    def _smc_inner_loop(self, i, t, k, Phi_k, A_k, Api_k, y_t, z_t, all_log_lambda_prev, all_ans_prev):
        ...
        if t == 0:
            ...
            idx = all_ans_prev[i]
            log_lambda_prev = all_log_lambda_prev[idx]
    ...
    def smc_step_single_parallel(self, t, k):
        ...
        all_log_lambda_prev = self.trajectories[:, :, k, t].compute()
        all_ans_prev = self.ancestors[:, :, k, t-1].compute().astype(int)

        ll_k, new_trj , new_ans= Parallel(n_jobs=-1, backend='threading')(
